function [ JaP_hg ] = JacAdjP_hg(volume,data,hg )
%JACADJP_HG - Applies the Jacobiant Adjoint operator to the increment hg.
%TO be used during Probe retireval
...
% JaP_hg              - Result after applying the linear operator JP' to hg
%
...
if~isfield(data,'mask_dc')
F2=Ift2(PHI0.*real(hg));
else
    if data.mask_dc
        mask_dc=ones(size(hg(:,:,1)));
        mask_dc(1)=0;
        mask_dc=fftshift(mask_dc);
        F2=Ift2((PHI0+1).*real(hg).*repmat(mask_dc,[1,1,size(hg,3)]));
    else
        F2=Ift2(PHI0.*real(hg));
    end
end

JaP_hg=2*sum(F1.*F2,3);
JaP_hg(isnan(JaP_hg))=0;
